...
def _build_prompt(
    *,
    ...
    return (
        ...
        "— Цены ТОЛЬКО в рублях, ТОЛЬКО ПРОПИСЬЮ. Базовая форма: "
        f"«{premium_price_str}», «{mimic_price_str}». НЕ цифрами «50000» "
        "или «50 тысяч». СКЛОНЯЙ по падежу контекста: после «вместо» — "
        "родительный («вместо пятидесяти тысяч рублей»); после «за» — "
        "родительный («за тысячу рублей»); после «отдала» — винительный "
        "(«отдала пятьдесят тысяч»). Числа МЕНЯТЬ нельзя, но падеж "
        "склоняй естественно как сказала бы носительница языка.\n"
